...
class jaw_default_widget extends WP_Widget {
    ...
    function update($new_instance, $old_instance) {
        ...
        foreach ($this->options as $one_option) {
            ...
            }else if ($one_option['type'] == 'menu-items') {
                $instance[$key] = $new_instance[$key];
            }
        }
        do_action('jaw_update_widget');
        return $instance;
    }
    ...
    function form($instance) {
        ...
        foreach ($this->options as $one_option) {
            ...
            if ($one_option['type'] == 'text') {
                ...
                $value = '';
                if (!isset($instance[$one_option['id']]) && $one_option['default'] == 1) {
                    $value = 'checked="checked"';
                }
                if (isset($instance[$one_option['id']]) && $instance[$one_option['id']] == 1) {
                    $value = 'checked="checked"';
                }
                echo '<p>';
